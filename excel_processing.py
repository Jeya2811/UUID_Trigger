...
from database import get_db_connection, get_deterministic_payer_id, get_deterministic_plan_id
...
def get_or_create_payer(cursor, payer_data):
    ...
    master_payer_id = get_deterministic_payer_id(cursor, payer_name)

    if result:
        existing_id = result[0]
        logger.debug(f"Found existing payer: {payer_name} (ID: {existing_id})")
        
        # If existing ID doesn't match master ID, we could update it here, 
        # but that requires cascading updates to all related tables.
        # For now, we return existing_id to maintain database integrity,
        # but we should rely on sync_payer_ids.py for the migration.
    ...
    # Create new payer using Master ID (deterministic)
    payer_id = master_payer_id
    try:
        cursor.execute("""
...
def get_or_create_plan(cursor, plan_data, payer_id):
    ...
    # Create new plan
    plan_id = get_deterministic_plan_id(cursor, payer_name, plan_name, state_name)
    try:
        cursor.execute("""
