...
def get_deterministic_payer_id(cursor, payer_name):
    """
    Get Payer_ID from payer_master table if exists, 
    otherwise generate a deterministic UUID based on the name.
    """
    if not payer_name:
        return None
    
    clean_name = str(payer_name).strip()
    
    # 1. Try to fetch from payer_master
    cursor.execute('SELECT "Payer_ID" FROM payer_master WHERE LOWER(TRIM("Payer_Name")) = LOWER(TRIM(%s))', (clean_name,))
    result = cursor.fetchone()
    
    if result:
        return result[0]
    
    # 2. Fallback to deterministic UUID (matches master script logic)
    return str(uuid.uuid5(uuid.NAMESPACE_OID, clean_name))

def get_deterministic_plan_id(cursor, payer_name, plan_name, state_name):
    """
    Look up PP_ID from payer_plan_master based on payer, plan, and state.
    If not found, generate a deterministic UUID.
    """
    if not payer_name or not plan_name:
        return None
        
    clean_payer = str(payer_name).strip()
    clean_plan = str(plan_name).strip()
    clean_state = str(state_name).strip() if state_name else ""
    
    # 1. Try to fetch from payer_plan_master
    cursor.execute("""
        SELECT "PP_ID" FROM payer_plan_master 
        WHERE LOWER(TRIM("Payer_Name")) = LOWER(TRIM(%s))
        AND LOWER(TRIM("PLAN_NAME")) = LOWER(TRIM(%s))
        AND LOWER(TRIM("STATE_NAME")) = LOWER(TRIM(%s))
    """, (clean_payer, clean_plan, clean_state))
    
    result = cursor.fetchone()
    if result:
        return result[0]
    
    # 2. Fallback to deterministic generation (matching plan_master.py logic)
    unique_key = f"{clean_payer}|{clean_plan}|{clean_state}"
...
def ensure_database_schema():
    ...
    with get_db_connection() as conn:
        ...
        try:
            ...
            logger.debug(f"pp_formulary_names table creation issue (may already exist): {e}")
            conn.rollback()
